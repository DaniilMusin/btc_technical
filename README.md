...
В репозитории присутствует файл `.pre-commit-config.yaml` с настройками хуков
`black`, `ruff`, `mypy` и `pytest`. Чтобы подключить их локально,
установите пакет `pre-commit` и активируйте его:
...
Теперь проверки будут выполняться при каждом коммите. Запустить их вручную на
всём проекте можно командой:
