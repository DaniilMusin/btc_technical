...
class MexcBroker:
    ...
    Если USE_TESTNET=true – метод place_market просто пишет в лог вместо
    отправки.
    """
    ...
    def __init__(self, testnet: bool = True):
        ...
        if testnet:
            logger.warning(
                "MEXC Broker in DRY‑RUN / TESTNET mode – сделки НЕ"
                " отправляются"
            )
    ...
    def _sign(self, params: dict) -> dict:
        ...
        signature = hmac.new(
            self.secret, qs.encode(), hashlib.sha256
        ).hexdigest()
    ...
    async def place_market(self, symbol: str, side: str, qty: float) -> dict:
        ...
        """
        if self.testnet:
            # В тестовом режиме используем «нулевой» resp, но ставим
            # fictive‑price
