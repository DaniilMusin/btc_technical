...
class BalancedAdaptiveStrategy:
    def __init__(
        ...
        """Initialization of balanced adaptive strategy for BTC futures
        trading"""
    ...
    def _notify_trade_open(self, trade: dict) -> None:
        msg = (
            ...
            f"SL: `{
                trade['stop_loss']:.2f}`  TP: `{
                trade['take_profit']:.2f}`\n"
            f"Вес сигнала: `{
                trade['weight']:.2f}`  Режим: `{
                trade['market_regime']}`"
        )
        tg_send(msg)
    ...
    def load_data(self) -> Optional[pd.DataFrame]:
        ...
        приводит индексы и имена колонок к нужному виду.
        """
        import warnings

        if not self.data_path:
            warnings.warn(
                "data_path не задан, load_data() ничего не делает",
                RuntimeWarning,
            )
            return self.data
        ...
        if date_col is None:
            raise ValueError(
                "Не найдена колонка с датой/временем в файле данных"
            )
    ...
    def validate_data(self) -> None:
        ...
        if missing:
            raise ValueError(
                f"В данных отсутствуют необходимые колонки: {missing}"
            )
        return
    ...
    def optimize_memory(self) -> None:
        ...
        bool_cols = [
            ...
            "Bullish_Divergence",
            "Bearish_Divergence",
            "Balanced_Long_Signal",
            "Balanced_Short_Signal",
            "MR_Long_Signal",
            "MR_Short_Signal",
            "Strong_Trend",
            "Weak_Trend",
    ...
    def calculate_indicators(self) -> pd.DataFrame:
        ...
        self.data[f"EMA_{short_ema}"] = self._calculate_ema(
            self.data["Close"], short_ema
        )
        self.data[f"EMA_{long_ema}"] = self._calculate_ema(
            self.data["Close"], long_ema
        )

        # --- RSI ---
        self.data["RSI"] = self._calculate_rsi(
            self.data["Close"], self.params["rsi_period"]
        )
        ...
        self.data["BB_Middle"] = (
            self.data["Close"].rolling(window=bb_period).mean()
        )
        ...
        # (4) Стабильная колонка для деления
        self.data["ATR_MA"].replace(0, 1e-10, inplace=True)

        # --- ADX ---
        adx_period = self.params["adx_period"]
        adx_results = self._calculate_adx(
            self.data["High"], self.data["Low"], self.data["Close"], adx_period
        )
        ...
        self.data["Price_Min"] = (
            self.data["Close"].rolling(5, center=True).min()
            == self.data["Close"]
        )
        self.data["Price_Max"] = (
            self.data["Close"].rolling(5, center=True).max()
            == self.data["Close"]
        )
        self.data["RSI_Min"] = (
            self.data["RSI"].rolling(5, center=True).min() == self.data["RSI"]
        )
        self.data["RSI_Max"] = (
            self.data["RSI"].rolling(5, center=True).max() == self.data["RSI"]
        )
        ...
        for i in range(1, len(price_mins)):
            for j in range(1, len(rsi_mins)):
                delta_sec = (
                    pd.Timestamp(rsi_mins[j]) - pd.Timestamp(price_mins[i])
                ).total_seconds()
                if 0 <= delta_sec / 3600 <= 3:
                    price_change = (
                        self.data.loc[price_mins[i], "Close"]
                        - self.data.loc[price_mins[i - 1], "Close"]
                    )
                    rsi_change = (
                        self.data.loc[rsi_mins[j], "RSI"]
                        - self.data.loc[rsi_mins[j - 1], "RSI"]
                    )
                    if price_change < 0 and rsi_change > 0:
                        self.data.loc[
                            max(price_mins[i], rsi_mins[j]),
                            "Bullish_Divergence",
                        ] = True
        ...
        for i in range(1, len(price_maxs)):
            for j in range(1, len(rsi_maxs)):
                delta_sec = (
                    pd.Timestamp(rsi_maxs[j]) - pd.Timestamp(price_maxs[i])
                ).total_seconds()
                if 0 <= delta_sec / 3600 <= 3:
                    price_change = (
                        self.data.loc[price_maxs[i], "Close"]
                        - self.data.loc[price_maxs[i - 1], "Close"]
                    )
                    rsi_change = (
                        self.data.loc[rsi_maxs[j], "RSI"]
                        - self.data.loc[rsi_maxs[j - 1], "RSI"]
                    )
                    if price_change > 0 and rsi_change < 0:
                        self.data.loc[
                            max(price_maxs[i], rsi_maxs[j]),
                            "Bearish_Divergence",
                        ] = True
        ...
        # Trend weight
        self.data["Trend_Weight"] = np.minimum(
            1.0,
            np.maximum(
                0,
        ...
        self.data["Daily_EMA50"] = self._calculate_ema(
            self.data["Daily_Close"], 50
        ).ffill()
        self.data["Daily_EMA200"] = self._calculate_ema(
            self.data["Daily_Close"], 200
        ).ffill()
        self.data["Higher_TF_Bullish"] = (
            self.data["Daily_EMA50"] > self.data["Daily_EMA200"]
        )
        ...
        self.data["HH"] = self.data["High"].rolling(10).max() > self.data[
            "High"
        ].rolling(20).max().shift(10)
        self.data["HL"] = self.data["Low"].rolling(10).min() > self.data[
            "Low"
        ].rolling(20).min().shift(10)
        self.data["LH"] = self.data["High"].rolling(10).max() < self.data[
            "High"
        ].rolling(20).max().shift(10)
        self.data["LL"] = self.data["Low"].rolling(10).min() < self.data[
            "Low"
        ].rolling(20).min().shift(10)
        ...
        self.data["Volume_MA_10"] = (
            self.data["Volume"].rolling(window=10).mean()
        )
        self.data["Rising_Volume"] = (
            self.data["Volume"] > self.data["Volume_MA_3"] * 1.2
        )
        self.data["Falling_Volume"] = (
            self.data["Volume"] < self.data["Volume_MA_3"] * 0.8
        )
        ...
        # Volatility ratio
        self.data["Volatility_Ratio"] = self.data["ATR"] / self.data[
            "ATR_MA"
        ].replace(0, 1e-10)

        # 1. Enhanced Market Regime Detection
        self.detect_market_regime(
            lookback=self.params["regime_volatility_lookback"]
        )

        # 2. Add Counter-Cyclical Indicators
        self.data["Price_to_200MA_Ratio"] = (
            self.data["Close"] / self.data["Daily_EMA200"]
        )
        self.data["Extreme_Overbought"] = (
            self.data["Price_to_200MA_Ratio"] > 1.3
        )
        self.data["Extreme_Oversold"] = self.data["Price_to_200MA_Ratio"] < 0.7

        # Add momentum oscillator divergence
        self.data["Momentum"] = self.data["Close"] - self.data["Close"].shift(
            14
        )
        self.data["Momentum_SMA"] = (
            self.data["Momentum"].rolling(window=10).mean()
        )
        ...
        logging.info(
            f"After dropna, {len(self.data)} rows remain for backtest."
        )
    ...
    def detect_market_regime(self, lookback=100):
        ...
        for i in range(lookback, len(self.data)):
            recent = self.data.iloc[i - lookback : i + 1]  # noqa: E203
            ...
            vol_expanding = (
                short_vol > medium_vol > long_vol
                if (medium_vol > 0 and long_vol > 0)
                else False
            )
            vol_contracting = (
                short_vol < medium_vol < long_vol
                if (short_vol > 0 and medium_vol > 0)
                else False
            )

            # Identify regimes
            if (
                short_direction > 0.05
                and medium_direction > 0.03
                and vol_expanding
            ):
                self.data.at[self.data.index[i], "Market_Regime"] = (
                    "strong_bull"
                )
            elif (
                short_direction < -0.05
        ...
        for regime, multipliers in regime_multipliers.items():
            ...
            self.data.loc[mask, "Regime_Short_Multiplier"] = multipliers[
                "SHORT"
            ]
    ...
    def calculate_multi_timeframe_confirmation(self):
        ...
        try:
            ...
            hourly_ema_fast = self._calculate_ema(
                hourly["close"], self.params["hourly_ema_fast"]
            )
            hourly_ema_slow = self._calculate_ema(
                hourly["close"], self.params["hourly_ema_slow"]
            )
            hourly_signal = hourly_ema_fast > hourly_ema_slow
            four_hour = self.data["Close"].resample("4H").ohlc()
            four_hour_ema_fast = self._calculate_ema(
                four_hour["close"], self.params["four_hour_ema_fast"]
            )
            four_hour_ema_slow = self._calculate_ema(
                four_hour["close"], self.params["four_hour_ema_slow"]
            )
            four_hour_signal = four_hour_ema_fast > four_hour_ema_slow
            self.data["Hourly_Bullish"] = (
                self.data.index.floor("H").map(hourly_signal).fillna(False)
            )
            self.data["4H_Bullish"] = (
                self.data.index.floor("4H").map(four_hour_signal).fillna(False)
            )
            ...
            self.data["MTF_Bull_Strength"] = self.data[
                "Higher_TF_Bullish"
            ].astype(int)
            self.data["MTF_Bear_Strength"] = self.data[
                "Higher_TF_Bearish"
            ].astype(int)
    ...
    def calculate_mean_reversion_signals(self):
        ...
        self.data["Close_SMA20"] = (
            self.data["Close"].rolling(window=lookback).mean()
        )
        self.data["Price_Deviation"] = (
            self.data["Close"] - self.data["Close_SMA20"]
        )
        self.data["Price_Deviation_Std"] = (
            self.data["Price_Deviation"].rolling(window=lookback).std()
        )
        self.data["Z_Score"] = self.data["Price_Deviation"] / self.data[
            "Price_Deviation_Std"
        ].replace(0, np.nan)
        ...
        self.data["MR_Long_Signal"] = (self.data["Z_Score"] < -threshold) & (
            self.data["Z_Score"].shift(1) >= -threshold
        )
        self.data["MR_Short_Signal"] = (self.data["Z_Score"] > threshold) & (
            self.data["Z_Score"].shift(1) <= threshold
        )
    ...
    def identify_market_cycle_phase(self):
        ...
        self.data.loc[
            self.data["Cycle_Phase"] == "Accumulation", "Long_Phase_Weight"
        ] = 1.3
        self.data.loc[
            self.data["Cycle_Phase"] == "Accumulation", "Short_Phase_Weight"
        ] = 0.7

        self.data.loc[
            self.data["Cycle_Phase"] == "Markup", "Long_Phase_Weight"
        ] = 1.5
        self.data.loc[
            self.data["Cycle_Phase"] == "Markup", "Short_Phase_Weight"
        ] = 0.5

        self.data.loc[
            self.data["Cycle_Phase"] == "Distribution", "Long_Phase_Weight"
        ] = 0.7
        self.data.loc[
            self.data["Cycle_Phase"] == "Distribution", "Short_Phase_Weight"
        ] = 1.3

        self.data.loc[
            self.data["Cycle_Phase"] == "Markdown", "Long_Phase_Weight"
        ] = 0.5
        self.data.loc[
            self.data["Cycle_Phase"] == "Markdown", "Short_Phase_Weight"
        ] = 1.5

    def calculate_market_health(self):
        """Calculate overall market health score (0-100)"""
        self.data["Trend_Health"] = (
            self.data["Close"] > self.data["Daily_EMA50"]
        ).astype(int) * 20

        vol_ratio = self.data["ATR"] / self.data["ATR"].rolling(
            100
        ...
        indicators_bullish = (
            (
                ...
                + (
                    self.data[f'EMA_{self.params["short_ema"]} ']
                    > self.data[f'EMA_{self.params["long_ema"]} ']
                ).astype(int)
        ...
        bb_position = (self.data["Close"] - self.data["BB_Lower"]) / (
            self.data["BB_Upper"] - self.data["BB_Lower"]
        )
        bb_position = bb_position.replace([np.inf, -np.inf], np.nan).fillna(
            0.5
        )
        self.data["Support_Resistance_Health"] = (
            (0.5 - abs(bb_position - 0.5)) * 2 * 20
        )

        self.data["Market_Health"] = (
            self.data["Trend_Health"] * self.params["health_trend_weight"]
            + self.data["Volatility_Health"]
            * self.params["health_volatility_weight"]
            + self.data["Volume_Health"] * self.params["health_volume_weight"]
            + self.data["Breadth_Health"]
            * self.params["health_breadth_weight"]
            + self.data["Support_Resistance_Health"]
            * self.params["health_sr_weight"]
        ).clip(0, 100)
    ...
    def calculate_momentum_metrics(self):
        ...
        for period in periods:
            self.data[f"ROC_{period}"] = (
                self.data["Close"].pct_change(period) * 100
            )

        momentum_components = [
            (
                np.sign(self.data[f"ROC_{period}"])
                * self.data[f"ROC_{period}"].abs() ** 0.5
            )
            for period in periods
        ]
        ...
        if max_val > 0:
            self.data["Momentum_Score"] = self.data["Momentum_Score"] * (
                100 / max_val
            )
        ...
        self.data["Potential_Momentum_Reversal"] = (
            (self.data["Momentum_Score"] > 80)
            & (self.data["Mom_Acceleration"] < -reversal_threshold)
        ) | (
            (self.data["Momentum_Score"] < -80)
            & (self.data["Mom_Acceleration"] > reversal_threshold)
        )
    ...
    def adapt_to_market_conditions(self):
        """Apply all market condition metrics to create balanced strategy
        bias"""
    ...
    def adaptive_risk_per_trade(
        self, current_market_regime, win_rate_long, win_rate_short
    ):
        ...
        if (
            hasattr(self, "trade_df")
            and self.trade_df is not None
            and len(self.trade_df) > 20
        ):
            last = self.trade_df.tail(20)
            win_rate = (last["pnl"] > 0).mean()
            profit_factor = (
                last[last["pnl"] > 0]["pnl"].sum()
                / abs(last[last["pnl"] <= 0]["pnl"].sum())
                if (last["pnl"] <= 0).any()
                else 2
            )
            risk_multiplier = 1.0
        ...
        return {
            "LONG": base_risk
            * long_adjustment
            * regime_factors[current_market_regime]["LONG"],
            "SHORT": base_risk
            * short_adjustment
            * regime_factors[current_market_regime]["SHORT"],
        }
    ...
    def _calculate_adx(self, high, low, close, period):
        ...
        dx = 100 * (
            (plus_di - minus_di).abs() / (plus_di + minus_di).replace(0, 1e-10)
        )
    ...
    def get_trading_signals(self, current, previous, regime_type):
        ...
        # If Daily_EMA50 < Daily_EMA200 and not Higher_TF_Bullish, prohibit
        # only LONG
        if (
            "Daily_EMA50" in current
            and "Daily_EMA200" in current
            and current["Daily_EMA50"] < current["Daily_EMA200"]
            and not current["Higher_TF_Bullish"]
        ):
            prohibit_long = True
        else:
            prohibit_long = False
        # If Daily_EMA50 > Daily_EMA200 and not Higher_TF_Bearish, prohibit
        # only SHORT
        if (
            "Daily_EMA50" in current
            and "Daily_EMA200" in current
            and current["Daily_EMA50"] > current["Daily_EMA200"]
            and not current["Higher_TF_Bearish"]
        ):
            prohibit_short = True
        else:
        ...
        if current["Volume_Ratio"] > self.params["volume_threshold"]:
            volume_multiplier = min(
                2.0, current["Volume_Ratio"] / self.params["volume_threshold"]
            )
        ...
        if regime_type == "trend":
            if current["Trend_Weight"] > 0.45:  # was 0.55, now 0.45 for LONG
                # Long signals
                if (
                    previous[f"EMA_{short_ema}"] < previous[f"EMA_{long_ema}"]
                ) and (
                ...
                if (
                    ...
                    and not previous["Bullish_Trend"]
                    and current["Plus_DI"] > current["Minus_DI"] * 1.2
                ):
                    signal_weight = (
                        current["Trend_Weight"]
                        * 1.5
                    ...
                    long_signals.append(
                        ("Strong Bullish Trend", signal_weight)
                    )
                ...
                # Volume Breakout
                if (
                    current["High"] > previous["High"] * 1.002
                    and current["Volume_Ratio"] > 1.3
                    and current["Bullish_Trend"]
                ):
                    signal_weight = (
                        current["Trend_Weight"] * 1.4 * regime_multiplier_long
                    )
                    long_signals.append(("Volume Breakout", signal_weight))

                # Daily Golden Cross
                if (
                    current["Higher_TF_Bullish"]
                    and not previous["Higher_TF_Bullish"]
                ):
                    long_signals.append(("Daily Golden Cross", 1.5))

                if (
                    current["Balanced_Long_Signal"]
                    and current["Final_Long_Bias"] > 0.60
                ):
                    signal_weight = (
                        current["Final_Long_Bias"]
                        * 1.5
                        * regime_multiplier_long
                    )
                    long_signals.append(
                        ("Balanced Long Signal", signal_weight)
                    )

                # --- Enhanced Volume Breakout ---
                if (
                    current["Volume_Ratio"] > 1.3
                    and current["Close"] > previous["High"] * 1.001
                ):
                    signal_weight = (
                        current["Trend_Weight"] * 1.4 * regime_multiplier_long
                    )
                    long_signals.append(
                        ("Enhanced Volume Breakout", signal_weight)
                    )
                ...
                if (
                    ...
                    long_signals.append(
                        ("Support Level Bounce", signal_weight)
                    )
                ...
                if (
                    ...
                    and not previous["Bearish_Trend"]
                    and current["Minus_DI"] > current["Plus_DI"] * 1.2
                ):
                    signal_weight = (
                        current["Trend_Weight"]
                        * 1.5
                    ...
                    short_signals.append(
                        ("Strong Bearish Trend", signal_weight)
                    )
                ...
                if (
                    current["Balanced_Short_Signal"]
                    and current["Final_Short_Bias"] > 0.65
                ):
                    signal_weight = (
                        current["Final_Short_Bias"]
                        * 1.5
                        * regime_multiplier_short
                    )
                    short_signals.append(
                        ("Balanced Short Signal", signal_weight)
                    )

                # 52-week High Breakout
                rolling_max = self.data["High"].rolling(1440).max().shift(1)
                if (
                    current["High"] > rolling_max.loc[current.name]
                ) and current["Volume_Ratio"] > 1.5:
                    signal_weight = (
                        1.6 * health_factor_long * regime_multiplier_long
                    )
            ...
            if current["Range_Weight"] > 0.6:
                ...
                if (
                    ...
                    and current["Close"] < current["BB_Lower"] * 1.01
                ):
                    signal_weight = (
                        current["Range_Weight"]
                        * 1.3
                        * health_factor_long
                        * phase_factor_long
                        * regime_multiplier_long
                    ...
                    long_signals.append(
                        ("RSI Oversold + BB Lower", signal_weight)
                    )
                ...
                if current["Bullish_Divergence"] and current["RSI"] < 40:
                    ...
                    long_signals.append(
                        ("Strong Bullish Divergence", signal_weight)
                    )

                if (
                    current["Close"] > current["Open"]
                    and previous["Close"] < previous["Open"]
                    and current["Low"] > previous["Low"] * 0.998
                    and current["Volume"] > previous["Volume"] * 1.2
                ):
                    signal_weight = (
                        current["Range_Weight"]
                        * 1.2
                ...
                if (
                    ...
                    and current["Close"] > current["BB_Upper"] * 0.99
                ):
                    signal_weight = (
                        current["Range_Weight"]
                        * 1.3
                    ...
                    short_signals.append(
                        ("RSI Overbought + BB Upper", signal_weight)
                    )
                ...
                if current["Bearish_Divergence"] and current["RSI"] > 60:
                    ...
                    short_signals.append(
                        ("Strong Bearish Divergence", signal_weight)
                    )

                if (
                    current["Close"] < current["Open"]
                    and previous["Close"] > previous["Open"]
                    and current["High"] < previous["High"] * 1.002
                    and current["Volume"] > previous["Volume"] * 1.2
                ):
                    signal_weight = (
                        current["Range_Weight"]
                    ...
                    short_signals.append(
                        ("Resistance Rejection", signal_weight)
                    )
                ...
                if current["MR_Short_Signal"] and current["Z_Score"] > 2.0:
                    ...
                    short_signals.append(
                        ("Mean Reversion Short", signal_weight)
                    )

        long_signals = [
            (signal, weight * volume_multiplier)
        ...
        if "prohibit_short" in locals() and prohibit_short:
            short_signals = []
        long_weight = (
            sum(weight for _, weight in long_signals) / len(long_signals)
            if long_signals
            else 0
        )
        short_weight = (
            sum(weight for _, weight in short_signals) / len(short_signals)
            if short_signals
            else 0
        )
    ...
    def apply_advanced_filtering(self, current, signals):
        ...
        atr_ma = (
            current["ATR_MA"]
            if (current["ATR_MA"] != 0 and not pd.isna(current["ATR_MA"]))
            else 1e-10
        )
        ...
        if (
            ...
            if len(long_trades) > 0:
                long_win_rate = sum(
                    1 for p in long_trades["pnl"] if p > 0
                ) / len(long_trades)
            ...
            if len(short_trades) > 0:
                short_win_rate = sum(
                    1 for p in short_trades["pnl"] if p > 0
                ) / len(short_trades)
        ...
        if "Market_Regime" in current:
            ...
            if regime == "strong_bull":
                ...
                if not any(
                    "Mean Reversion" in signal
                    for signal, _ in signals["long_signals"]
                ):
                    long_weight *= 0.8
                if not any(
                    "Mean Reversion" in signal
                    for signal, _ in signals["short_signals"]
                ):
                    short_weight *= 0.8
        ...
        if (
            "Higher_TF_Bullish" in current
            and current["Higher_TF_Bullish"]
            and current["Daily_EMA50"] > current["Daily_EMA200"]
        ):
            short_weight *= 0.3
        ...
        if (
            ...
            short_ratio = (
                len(recent_trades[recent_trades["position"] == "SHORT"])
                / len(recent_trades)
        ...
        short_weight *= self.params["global_short_penalty"]
        # --- Apply global long boost ---
        long_weight *= self.params["global_long_boost"]
        # если дневная EMA-50 > EMA-200 и цена выше EMA-50 +- 0.5 % → почти не
        # шортим
        if (
            current["Higher_TF_Bullish"]
            and current["Close"] > current["Daily_EMA50"] * 1.005
        ):
            short_weight *= 0.30  # было 0.05
        # --- hot-fix: вернуть агрессию шортам ---
        # теперь параметр
        short_weight *= self.params.get("short_hotfix_multiplier", 1.0)
        ...
        return {"long_weight": long_weight, "short_weight": short_weight}

    def calculate_dynamic_exit_levels(
        self, position_type, entry_price, current_candle, trade_age_hours=0
    ):
        # --- Volatility scaling for SL/TP ---
        atr_ma = (
            current_candle["ATR_MA"]
            if (
                "ATR_MA" in current_candle
                and not pd.isna(current_candle["ATR_MA"])
                and current_candle["ATR_MA"] != 0
            )
            else 1e-10
        )
        ...
        if position_type == "LONG":
            stop_loss = entry_price * (
                1 - atr_value * sl_multiplier / entry_price
            )
            take_profit = entry_price * (
                1 + atr_value * tp_multiplier / entry_price
            )
        else:
            stop_loss = entry_price * (
                1 + atr_value * sl_multiplier / entry_price
        ...
        if (
            "Range_Weight" in current_candle
            and current_candle["Range_Weight"] > 0.7
        ):
            if (
                position_type == "LONG"
                and current_candle["BB_Upper"] < take_profit
            ):
                take_profit = current_candle["BB_Upper"]
            elif (
                position_type == "SHORT"
                and current_candle["BB_Lower"] > take_profit
            ):
    ...
    def calculate_optimal_leverage(
        self, current_candle, trade_direction, max_allowed_leverage=3
    ):
        base_leverage = 2
        ...
        atr_ma = (
            current_candle["ATR_MA"]
            if (
                not pd.isna(current_candle["ATR_MA"])
                and current_candle["ATR_MA"] != 0
            )
            else 1e-10
        )
        ...
        if current_candle["ADX"] > 35:
            if (
                ...
                and current_candle["Plus_DI"] > current_candle["Minus_DI"]
            ) or (
                trade_direction == "SHORT"
                and current_candle["Minus_DI"] > current_candle["Plus_DI"]
            ):
    ...
    def adaptive_position_sizing(
        ...
        base_position_size = (
            risk_amount / price_risk_pct if price_risk_pct > 0 else 0
        )
        # Гибкий минимум позиции
        min_pos = (
            max(MIN_POSITION, balance * risk_per_trade / price_risk_pct)
            if price_risk_pct > 0
            else MIN_POSITION
        )
        ...
        if (
            position_size == min_pos
            and position_size > balance * optimal_leverage
        ):
            warnings.warn(
                f"Position size {
                    position_size:.2f} > max allowed {
                    balance *
                    optimal_leverage:.2f} по плечу! Ограничено сверху."
            )
        elif position_size == min_pos:
            warnings.warn(
                f"Position size {
                    position_size:.2f} < min allowed {
                    min_pos:.2f}. Risk per trade может быть превышен!"
            )
    ...
    def dynamically_adjust_risk_parameters(self):
        ...
        if not (
            hasattr(self, "trade_df")
            and isinstance(self.trade_df, pd.DataFrame)
            and not self.trade_df.empty
        ):
            # print("Trade DataFrame not initialized or missing data")
            return
        recent_trades = self.trade_df.tail(50)

        win_rate = sum(1 for p in recent_trades["pnl"] if p > 0) / len(
            recent_trades
        )
        ...
        if profit_factor > 1.5 and win_rate > 0.5:
            self.base_risk_per_trade = min(
                0.026, self.base_risk_per_trade * 1.3
            )
        elif profit_factor < 1.0 or win_rate < 0.4:
            self.base_risk_per_trade = max(
                0.014, self.base_risk_per_trade * 0.7
            )

        if avg_win > 0 and avg_loss > 0:
            current_rr_ratio = avg_win / avg_loss
            if current_rr_ratio < 1.5:
                self.params["atr_multiplier_tp"] = min(
                    7.0, self.params["atr_multiplier_tp"] * 1.05
                )
            elif current_rr_ratio > 3.0 and win_rate < 0.4:
                self.params["atr_multiplier_tp"] = max(
                    3.0, self.params["atr_multiplier_tp"] * 0.95
                )
        ...
        long_win_rate = (
            sum(1 for p in long_trades["pnl"] if p > 0) / len(long_trades)
            if len(long_trades) > 0
            else 0.5
        )
        short_win_rate = (
            sum(1 for p in short_trades["pnl"] if p > 0) / len(short_trades)
            if len(short_trades) > 0
            else 0.5
        )
    ...
    def analyze_hour_performance(self):
        if not hasattr(self, "trade_df") or len(self.trade_df) == 0:
            return None
        self.trade_df["entry_hour"] = pd.to_datetime(
            self.trade_df["entry_date"]
        ).dt.hour

        hour_stats = (
            self.trade_df.groupby("entry_hour")
            .agg({"pnl": ["count", "mean", "sum"], "position": "count"})
            .reset_index()
        )
        ...
        optimal_hours = hour_stats[
            (hour_stats["win_rate"] > 0.5) & (hour_stats["num_trades"] >= 5)
        ]["hour"].tolist()
        if optimal_hours:
    ...
    def analyze_day_performance(self):
        if not hasattr(self, "trade_df") or len(self.trade_df) == 0:
            return None
        self.trade_df["entry_day"] = pd.to_datetime(
            self.trade_df["entry_date"]
        ).dt.dayofweek
        ...
        day_stats = (
            self.trade_df.groupby("day_name")
            .agg({"pnl": ["count", "mean", "sum"], "position": "count"})
            .reset_index()
        )
        ...
        optimal_days = day_stats[
            (day_stats["win_rate"] > 0.5) & (day_stats["num_trades"] >= 5)
        ]["day"].tolist()
        if optimal_days:
    ...
    def calculate_correlation_metrics(self, benchmark_path=None):
        ...
        if (
            hasattr(self, "backtest_results")
            and "equity" in self.backtest_results.columns
        ):
            strategy_returns = (
                self.backtest_results["equity"].pct_change().fillna(0)
            )
            window = min(100, len(strategy_returns) // 4)
            min_length = min(
                len(strategy_returns), len(self.data["Benchmark_Return"])
            )
            strategy_returns = strategy_returns.iloc[:min_length]
            benchmark_returns = self.data["Benchmark_Return"].iloc[:min_length]

            rolling_corr = strategy_returns.rolling(window).corr(
                benchmark_returns
            )
            autocorr = pd.Series(
            ...
            if abs(avg_corr) < 0.3:
                print(
                    "Strategy shows low correlation with BTC price, "
                    "suggesting market-neutral characteristics"
                )
            elif avg_corr > 0.7:
                print(
                    "Strategy is highly correlated with BTC price, "
                    "may struggle in bear markets"
                )
            elif avg_corr < -0.7:
                print(
                    "Strategy is highly negatively correlated with BTC price, "
                    "may struggle in bull markets"
                )
    ...
    def run_backtest(self) -> Optional[pd.DataFrame]:
        ...
        for i in range(1, len(self.data)):
                ...
                if position == 1:
                    ...
                    if current["Low"] <= stop_loss_price:
                        pnl = self._close_position(
                            "LONG", position_size, entry_price, stop_loss_price
                        )
                        balance += pnl
                    ...
                    if (
                        pyramid_entries < self.params["max_pyramid_entries"]
                        and current["Bullish_Trend"]
                        and current["ADX"] > 40
                        and current["Close"]
                        > entry_price * (1 + self.params["pyramid_min_profit"])
                        and (
                            current["MACD_Bullish_Cross"]
                            or current["Final_Long_Bias"] > 0.7
                        )
                    ):

                        additional_size = (
                            position_size
                            * self.params["pyramid_size_multiplier"]
                        )
                        ...
                        exit_levels = self.calculate_dynamic_exit_levels(
                            "LONG", entry_price, current, trade_age_hours
                        )
                    ...
                    exit_signals = []
                    current_regime = (
                        "trend" if current["Trend_Weight"] > 0.5 else "range"
                    )

                    if current_regime == "trend":
                        if (
                            previous[f"EMA_{short_ema}"]
                            >= previous[f"EMA_{long_ema}"]
                        ) and (
                            current[f"EMA_{short_ema}"]
                            < current[f"EMA_{long_ema}"]
                        ):
                        ...
                        if (
                            current["Higher_TF_Bearish"]
                            and unrealized_pnl_pct > 0.02
                        ):
                        ...
                        if (
                            current["Stat_Overbought"]
                            and unrealized_pnl_pct > 0.03
                        ):
                    ...
                    if (
                        ...
                        and previous["MACD"] > current["MACD"]
                    ):
                        exit_signals.append("Profit Protection")

                    if (
                        "Potential_Momentum_Reversal" in current
                        and current["Potential_Momentum_Reversal"]
                        and unrealized_pnl_pct > 0.04
                    ):
                        exit_signals.append("Momentum Reversal")

                    if (
                        "Final_Short_Bias" in current
                        and current["Final_Short_Bias"] > 0.7
                        and unrealized_pnl_pct > 0.03
                    ):
                    ...
                    optimal_leverage = self.calculate_optimal_leverage(
                        current, "SHORT", self.max_leverage
                    )
                    ...
                    if (
                        pyramid_entries < self.params["max_pyramid_entries"]
                        and current["Bearish_Trend"]
                        and current["ADX"] > 40
                        and current["Close"]
                        < entry_price * (1 - self.params["pyramid_min_profit"])
                        and (
                            current["MACD_Bearish_Cross"]
                            or current["Final_Short_Bias"] > 0.7
                        )
                    ):
                        ...
                        exit_levels = self.calculate_dynamic_exit_levels(
                            "SHORT", entry_price, current, trade_age_hours
                        )
                        stop_loss_price = exit_levels["stop_loss"]
                        take_profit_price = exit_levels["take_profit"]
                    ...
                    if (
                        "Final_Long_Bias" in current
                        and current["Final_Long_Bias"] > 0.7
                        and unrealized_pnl_pct > 0.03
                    ):
                        exit_signals.append("Bias Shift")
            ...
            if position == 0:
                ...
                signals = self.get_trading_signals(
                    current, previous, current_regime
                )
                filtered_signals = self.apply_advanced_filtering(
                    current, signals
                )
                ...
                #
                if (
                    filtered_signals["long_weight"]
                    > filtered_signals["short_weight"]
                    and filtered_signals["long_weight"] >= long_entry_threshold
                ):
                    ...
                    signal_info = ", ".join(
                        signal for signal, _ in signals["long_signals"]
                    )
                    ...
                    filtered_signals["short_weight"]
                    > filtered_signals["long_weight"]
                    and filtered_signals["short_weight"]
                    ...
                    signal_info = ", ".join(
                        signal for signal, _ in signals["short_signals"]
                    )
            ...
            if (
                position == 1
                and "unrealized_pnl_pct" in locals()
                and unrealized_pnl_pct > 0.12
                and position_size > 0
            ):
                partial_size = position_size * 0.4
                partial_pnl = partial_size * (
                    (current["Close"] / entry_price) - 1
                )
            ...
            if (
                position == 1
                and pyramid_entries < self.params["max_pyramid_entries"]
                and current["Bullish_Trend"]
                and current["ADX"] > 30
                and current["Close"] > entry_price * (1 + 0.02)
                and (
                    current["MACD_Bullish_Cross"]
                    or current["Final_Long_Bias"] > 0.65
                )
            ):
                additional_size = (
                    position_size * self.params["pyramid_size_multiplier"]
                )
                old_value = entry_price * position_size
                new_value = current["Close"] * additional_size
                position_size += additional_size
        ...
        if position != 0 and entry_price > 0:
            ...
            trade_age_hours = (
                last_candle.name - entry_date
            ).total_seconds() / 3600

            if position == 1:
                pnl = self._close_position(
                    "LONG", position_size, entry_price, exit_price
                )
            else:
                pnl = self._close_position(
                    "SHORT", position_size, entry_price, exit_price
                )
            balance += pnl
        ...
        if self.trade_history:
            ...
            self.trade_df["exit_date"].fillna(
                self.data.index[-1], inplace=True
            )
            self.trade_df["exit_price"].fillna(
                self.data["Close"].iloc[-1], inplace=True
            )
            mask = self.trade_df["pnl"].isna()
            self.trade_df.loc[
                mask & (self.trade_df["position"] == "LONG"), "pnl"
            ] = self.trade_df.loc[
                mask & (self.trade_df["position"] == "LONG"), "position_size"
            ] * (
                self.trade_df.loc[
                    mask & (self.trade_df["position"] == "LONG"), "exit_price"
                ]
                / self.trade_df.loc[
                    mask & (self.trade_df["position"] == "LONG"), "entry_price"
                ]
                - 1
            )
            self.trade_df.loc[
                mask & (self.trade_df["position"] == "SHORT"), "pnl"
            ] = self.trade_df.loc[
                mask & (self.trade_df["position"] == "SHORT"), "position_size"
            ] * (
                1
                - self.trade_df.loc[
                    mask & (self.trade_df["position"] == "SHORT"), "exit_price"
                ]
                / self.trade_df.loc[
                    mask & (self.trade_df["position"] == "SHORT"),
                    "entry_price",
                ]
            )
        else:
    ...
    def rebalance_long_short_bias(self):
        if (
            not hasattr(self, "trade_df")
            or self.trade_df is None
            or len(self.trade_df) < 10
        ):
            return
        ...
        short_r = 1 - long_r
        if short_r > 0.7:
            # (2) Исправление обращения к словарю
            self.params["global_short_penalty"] = 0.7
        else:
    ...
    def plot_equity_curve(self):
        ...
        ax1.plot(
            self.backtest_results["date"],
            self.backtest_results["equity"],
            label="Equity",
            color="blue",
        )
        ax1.plot(
            self.backtest_results["date"],
            self.backtest_results["balance"],
            label="Balance",
            color="green",
        )
        ...
        ax2.fill_between(
            self.backtest_results["date"], 0, drawdown, color="red", alpha=0.3
        )
        ax2.set_ylim(bottom=0, top=max(drawdown) * 1.5)
        ax2.invert_yaxis()

        if (
            "market_health" in self.backtest_results.columns
            and not self.backtest_results["market_health"].isnull().all()
        ):
            ax3.plot(
                self.backtest_results["date"],
                self.backtest_results["market_health"],
                label="Market Health",
                color="purple",
                alpha=0.7,
            )
            ...
            if (
                "market_regime" in self.backtest_results.columns
                and not self.backtest_results["market_regime"].isnull().all()
            ):
                regime_indicator = (
                    self.backtest_results["market_regime"]
                    .map(
                        {
                            "strong_bull": 1.0,
        ...
        if (
            hasattr(self, "trade_df")
            and self.trade_df is not None
            and len(self.trade_df) > 0
        ):
            win_rate = (
                len(self.trade_df[self.trade_df["pnl"] > 0])
                / len(self.trade_df)
                * 100
            )
        else:
        ...
        textstr = "\n".join(
            (
                ...
                (
                    "Total Trades: "
                    f"{len(self.trade_df) if hasattr(self, 'trade_df') else 0}"
                ),
            )
        )
        ...
        if hasattr(self, "trade_df") and len(self.trade_df) > 0:
            sample_trades = (
                self.trade_df.sample(min(50, len(self.trade_df)))
                if len(self.trade_df) > 50
                else self.trade_df
            )
            for idx, trade in sample_trades.iterrows():
                ...
                closest_entry = min(
                    self.backtest_results["date"],
                    key=lambda x: abs(x - entry_date),
                )
                entry_equity = self.backtest_results.loc[
                    self.backtest_results["date"] == closest_entry, "equity"
                ].values
                ...
                closest_exit = min(
                    self.backtest_results["date"],
                    key=lambda x: abs(x - exit_date),
                )
                exit_equity = self.backtest_results.loc[
                    self.backtest_results["date"] == closest_exit, "equity"
                ].values
    ...
    def plot_regime_performance(self):
        if (
            not hasattr(self, "trade_df")
            or len(self.trade_df) == 0
            or "market_regime" not in self.trade_df.columns
        ):
        ...
        regime_stats = (
            self.trade_df.groupby("market_regime")
            .agg({"pnl": ["count", "mean", "sum"], "position": "count"})
            .reset_index()
        )
        ...
        for regime in regime_stats["regime"].unique():
            regime_trades = self.trade_df[
                self.trade_df["market_regime"] == regime
            ]
        ...
        bars1 = ax1.bar(
            regime_stats["regime"],
            regime_stats["num_trades"],
            color=[
                regime_colors.get(r, "gray") for r in regime_stats["regime"]
            ],
        )
    ...
    def analyze_results(self):
        ...
        if months_diff == 0:
            months_diff = 1
        monthly_return = (
            ((final_balance / initial_balance) ** (1 / months_diff)) - 1
        ) * 100
        equity_curve = self.backtest_results["equity"]
        running_max = equity_curve.cummax()
        drawdown = (running_max - equity_curve) / running_max * 100
        ...
        if "equity" in self.backtest_results.columns:
            ...
            period_returns = (
                self.backtest_results["equity"].pct_change().dropna()
            )
            if len(period_returns) > 0:
                # Приведём к годовому значению: 15-минутных периодов в году =
                # 4*24*365 = 35040
                periods_per_year = 4 * 24 * 365
                sharpe_ratio = (
                    (period_returns.mean() * periods_per_year)
                    / (period_returns.std() * (periods_per_year**0.5))
                    if period_returns.std() > 0
                    else 0
                )
                negative_returns = period_returns[period_returns < 0]
                downside_deviation = (
                    negative_returns.std() * (periods_per_year**0.5)
        ...
        if "trade_duration" in trades.columns:
            ...
            trades["duration"] = pd.to_datetime(
                trades["exit_date"]
            ) - pd.to_datetime(trades["entry_date"])
            median_duration_hours = (
                trades["duration"].median().total_seconds() / 3600
            )
        ...
        avg_long_pnl = (
            trades[trades["position"] == "LONG"]["pnl"].mean()
            if long_trades > 0
            else 0
        )
        avg_short_pnl = (
            trades[trades["position"] == "SHORT"]["pnl"].mean()
            if short_trades > 0
            else 0
        )
        long_win_rate = (
            len(trades[(trades["position"] == "LONG") & (trades["pnl"] > 0)])
            / long_trades
            * 100
            if long_trades > 0
            else 0
        ...
        short_win_rate = (
            len(trades[(trades["position"] == "SHORT") & (trades["pnl"] > 0)])
            / short_trades
            * 100
            if short_trades > 0
            else 0
        ...
        print(
            f"Short Trades: {short_trades} (Win Rate: {
                short_win_rate:.2f}%)"
        )
        ...
        self.backtest_results["year"] = pd.to_datetime(
            self.backtest_results["date"]
        ).dt.year
        yearly_performance = {}
        for year in self.backtest_results["year"].unique():
            year_data = self.backtest_results[
                self.backtest_results["year"] == year
            ]
        ...
        if "market_regime" in trades.columns:
            ...
            for _, row in regime_stats.iterrows():
                print(
                    f"{
                        row['regime']}: Win Rate {
                        row['win_rate']:.2f}%, Avg P&L ${
                        row['avg_pnl']:.2f}, Total P&L ${
                        row['total_pnl']:.2f}, Trades: {
                        row['num_trades']}"
                )
        if (
            "market_health" in trades.columns
            and not trades["market_health"].isnull().all()
        ):
            ...
            health_stats = (
                trades.groupby("health_bin")
                .agg({"pnl": ["count", "mean", "sum"], "position": "count"})
                .reset_index()
            )
            health_stats.columns = [
                "health_range",
                "num_trades",
                "avg_pnl",
                "total_pnl",
        ...
        if "pyramid_entries" in trades.columns:
            ...
            pyramid_win_rate = (
                len(pyramid_trades[pyramid_trades["pnl"] > 0])
                / len(pyramid_trades)
                * 100
                if len(pyramid_trades) > 0
                else 0
            )
            non_pyramid_win_rate = (
                len(non_pyramid_trades[non_pyramid_trades["pnl"] > 0])
                / len(non_pyramid_trades)
                * 100
                if len(non_pyramid_trades) > 0
                else 0
            )
            ...
            print("\n===== PYRAMIDING EFFECT =====")
            print(
                f"Trades with Pyramiding: {
                    len(pyramid_trades)} (Win Rate: {
                    pyramid_win_rate:.2f}%)"
            )
            print(
                f"Trades without Pyramiding: {
                    len(non_pyramid_trades)} (Win Rate: {
                    non_pyramid_win_rate:.2f}%)"
            )
        ...
        if day_stats is not None:
            ...
            for _, row in day_stats.sort_values(
                "win_rate", ascending=False
            ).iterrows():
                print(
                    f"{
    ...
    def optimize_parameters(self, param_ranges=None, num_trials=20):
        ...
        for trial in range(num_trials):
            ...
            # (9) Восстанавливаем параметры перед каждым тестом
            ...
            if profit > best_profit:
                ...
                print(
                    f"New best profit: ${
                        best_profit:.2f} (Trial {
                        trial + 1}/{num_trials})"
                )
            else:
                print(
                    f"Trial {
                        trial + 1}/{num_trials} - Profit: ${
                        profit:.2f} (Best: ${
                        best_profit:.2f})"
                )
    ...
    def _close_position(
        self, position_type, position_size, entry_price, exit_price
    ):
        """Calculate realized PnL with commission and slippage for LONG or
        SHORT"""
        commission = (
            position_size * COMMISSION_RATE
        )  # (7) Используем константу
        ...
        if position_type == "LONG":
            ...
            pnl = position_size * (1 - (exit_price / entry_price)) - (
                commission + slippage
            )
        return pnl
...
def main():
    ...
    if not csv_files:
        print(
            ...
            f"Please ensure your data file is in this directory."
        )
        return
