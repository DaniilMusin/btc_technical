...
class BalancedAdaptiveStrategyLive(BalancedAdaptiveStrategy):
    ...
    def on_new_candle(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.side:
            ...
            if self.side == "LONG" and cur["Close"] > self.entry * (
                1 + TRAIL_TRIGGER_LONG_PCT
            ):
                new_sl = max(self.sl, cur["Close"] * (1 - TRAIL_SL_LONG_PCT))
                if new_sl != self.sl:
                    logger.info(
                        "Update trailing SL (LONG) %.2f → %.2f",
                        self.sl,
                        new_sl,
                    )
                    self.sl = new_sl
            elif self.side == "SHORT" and cur["Close"] < self.entry * (
                1 - TRAIL_TRIGGER_SHORT_PCT
            ):
                new_sl = min(self.sl, cur["Close"] * (1 + TRAIL_SL_SHORT_PCT))
                if new_sl != self.sl:
                    logger.info(
                        "Update trailing SL (SHORT) %.2f → %.2f",
                        self.sl,
                        new_sl,
                    )
                    self.sl = new_sl
        ...
        long_ok = (
            filt["long_weight"] >= 0.65
            and filt["long_weight"] > filt["short_weight"]
        )
        short_ok = (
            filt["short_weight"] >= 0.65
            and filt["short_weight"] > filt["long_weight"]
        )

        if long_ok:
            lvls = self.calculate_dynamic_exit_levels(
                "LONG", cur["Close"], cur
            )
            return {
                "action": "BUY",
    ...
    def open_position(
        ...
        logger.success(
            "POS OPEN %s qty=%.4f EP=%.2f SL=%.2f TP=%.2f",
            side,
            qty,
            entry_price,
            sl,
    ...
    def close_position(self, exit_price: float, ts: dt.datetime, reason: str):
        ...
        logger.success(
            "POS CLOSE %s pnl=%.2f  reason=%s", self.side, pnl, reason
        )
    ...
    def calc_qty(self, balance: float, price: float, sl: float) -> float:
        ...
        рассчитываем %‑риск (по статистике) и размер позы в *BASE‑валюте*
        (например, BTC).
        """
